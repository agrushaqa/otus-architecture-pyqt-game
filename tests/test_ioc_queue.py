...
from features.steps.src.lib_for_tests.mysql import (delete_table,
                                                    list_tables_in_db)
from features.steps.src.queue.create_class import create_command_instance
...
class TestIocThread:
    ...
    def test_thread_ioc_soft_stop_with_command(self):
        ...
        thread.join()
        delete_table(conn, table_name)  # удаление таблицы может не работать
        # из-за блокировки доступа к базе другим процессом
