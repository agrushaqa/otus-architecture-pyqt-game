...
import features.steps.src.lib_for_tests.write_to_file_for_test \
    as write_to_file_for_test
from features.steps.src.commands.start_working_thread import (
    StartBlockedWorkingThread, ThreadConfig)
...
class TestWorkingThread:
    ...
    def test_command_thread(self, mkdir_tmp_if_not_exist):
        ...
        ioc.resolve("StartBlockedThreadCommand", thread_config
                    ).execute().execute()
        with open(path, 'r') as f:
            content = f.read()
        os.remove(path)
